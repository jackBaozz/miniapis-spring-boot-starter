...
- **Spring AOP**: When using Spring AOP, rely on `MethodSignature.getMethod()` provided by the join point rather than dynamically resolving methods via target class reflection. Note that `MethodSignature.getMethod()` returns a `java.lang.reflect.Method` type; importing and using this type to get annotations (e.g. `method.getAnnotation()`) is safe and allowed in GraalVM native images. The restriction applies to actively resolving methods dynamically.
